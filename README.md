...
#### 1. GameOver and Win screens music overlap
...
| `SettingsController`    | Manages game settings including background music and sound effects volume controls with real-time adjustment and persistent state management.                          |
...
| `LevelTwo`    | Second level of the story mode requiring 10 kills to advance, featuring enhanced enemy types with strategic spawn positions (Y-axis range 100 to maximum). Implements a 20% spawn probability system with a maximum of 7 concurrent enemies. |

###
#### <ins>4.1.7 Managers</ins>
| Class Name           | Description                                                                                                                                                                                                      |
|----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `BulletManager`      | Manages ammunition system with configurable magazine size and reload mechanics. Features automatic reload timers, reload sound effects, and pause/resume functionality for game state handling.                  |
...
| `LeaderboardManager` | Manages arcade mode high scores with persistent storage, maintaining top 10 scores in descending order. Provides score loading, saving, and retrieval functionality.                                             |
...
| `ArcadeLevelView`        | Extended view class for arcade mode UI, managing score display positioning and custom kill count visualization at specific screen coordinates (1100, 25).                                              |
| `BossHealthBar`          | Visual component displaying boss health status using a styled progress bar (300px width) with color-coded health representation and numerical overlay. Features dynamic health percentage calculation. |
...
| `KillCountDisplay`       | Base class for kill count visualization with configurable positioning and formatting. Uses styled labels (Arial Black, 25pt) for consistent text display.                                              |
| `LevelTransitionScreen`  | Manages level transition effects with fade animations (2-second duration), displaying styled level names (Arcade font, 48pt) against black background.                                                 |
...
#### 12. Applied singleton design pattern to `soundmanager` and `entitymanager`.
